...
issue_cert()
{
	...
	[ -n "$webroot" ] || [ -n "$dns" ] || [ "$tls" -eq "1" ] || pre_checks "$main_domain" || return 2
    fi
	...
	if [ ! -f  "$STATE_DIR/private/key.pem" ]; then
	    ...
	    $ACME $debug --confdir "$STATE_DIR" --type EC --bits 384 $staging --yes new $ACCOUNT_EMAIL
	fi
    ...
    # uacme handles ECC key args differently
    if [ "$APP" = "uacme" ]; then
	if [ "${keylength:0:2}" = "ec" ]; then
	    acme_args="$acme_args --type EC --bits ${keylength:2}"
	else
...
#TO-DO
	if [ "$APP" = "acme" ]; then
	    ...
	    log "Using dns mode with dns-01"
	    # load hook program from config
	    config_get config_hprogram "$section" uacme_hook ""
	    if [ -n "${config_hprogram}" ]; then
	    	HPROGRAM="${config_hprogram}"
	    fi
	    # export section name for hook to use
	    export UACME_UCI_SECTION="$section"
